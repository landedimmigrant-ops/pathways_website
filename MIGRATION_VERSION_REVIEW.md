...
| 33 | Low | **✅ FIXED (iter 15, core)** — `<main id="app" aria-live="polite">` over-announced routine UI updates to SRs | Removed the broad `aria-live` from `<main>`; added a **scoped** `aria-live="polite" aria-atomic="true"` on `.results-meta` so filter/search changes announce just "Results: N". Verified: `#app` has no live region, the count updates **in place** (same node, 24→1 on search) so the scoped region fires. *SR-announce behaviour to confirm with a real screen reader, and route-change H1-focus is a further enhancement — both for the AEM a11y pass.* |
...
| 38 | Low | **✅ FIXED (iter 16, defensive)** — Booking/waitlist form accepted a malformed email (confirmation promised follow-up "at asdf") | Added an `emailInput.checkValidity()` guard in the booking submit handler (after the emptiness check), reusing `.is-invalid` + `reportValidity()`. *Latent: the in-page booking form is only reached for `bookingUrl`-less items, of which the live catalogue currently has none — not browser-verifiable now, but activates with the booking-data migration.* |
| 39 | Low | **↩︎ NOT FIXED — target is dead code (see #43)** — "Feedback modal never validates its optional email" | The in-app feedback modal (`openFeedbackModal`) is **never invoked** — the footer "Send feedback" opens an external MS Form instead. Adding validation to an unreachable function is pointless; reverted. Logged the dead-code finding as **#43** for the AEM team to wire-or-drop. |
...
| 43 | Low (code health) | **NEW (iter 16)** — `openFeedbackModal` is dead code (defined, never called) | The full in-app feedback modal (Formspree-wired, styled) is unreachable — the footer "Send feedback" goes to an external MS Form. Same class as the removed #17 Quick Match. AEM team: either wire it to a trigger or drop `openFeedbackModal` (app.js ~1273) so the handover doesn't carry dead UI. *(Not removed unilaterally — whether the in-app modal should supersede the MS Form is a product call.)* |
...
## Iteration 15 (2026-06-13) — scope the over-broad `aria-live` (#33 core) — asset `?v=158`

- **#33 (core) — over-broad `aria-live` on `<main>`.** `<main id="app" aria-live="polite">` made screen readers re-announce **every** dynamic update inside the whole app (cards, panels, accordions). Removed it and added a **scoped** live region (`aria-live="polite" aria-atomic="true"`) on `.results-meta`, so the one update that matters — the filter/search result count — still announces ("Results: N"). **Verified:** `#app` has no `aria-live`; `.results-meta` carries the scoped region; the count node updates **in place** (24→1 on a "narrative" search, same DOM node) so the live region will fire; no console errors across 6 routes. *The actual SR announcement and the optional route-change H1-focus enhancement are left for the AEM SGQRI 008 / WCAG pass.* `app.js` + `index.html`; assets → `?v=158`.

**Documented-Lows progress: 8 of 14 fixed** (#29, #30, #32, #33, #34, #35, #36, #40). The remaining items are either product/behavioral choices (**#31**), coordinator-owned data (**#37**), latent-in-current-data forms (**#38/#39**), or accepted-open NCV builder (**#41/#42** under **#12**) — i.e. I've now cleared everything I can responsibly auto-fix on a frozen build. Further passes return to verification/idle unless you reprioritize. **Unpushed stack: 7 commits** (`7fe58bf` → this) — recommend a push/review.

---

## Iteration 16 (2026-06-13) — booking email validation (#38) + dead-code finding (#43) — asset `?v=159`

- **#38 (defensive) — booking/waitlist form malformed-email guard.** Added `emailInput.checkValidity()` after the emptiness check in the booking submit handler (reusing `.is-invalid` + `reportValidity()`), so a typo'd address can't produce a "follow-up at <garbage>" confirmation. **Latent:** the in-page booking form is only reached for `bookingUrl`-less items, of which the live catalogue has none — not browser-verifiable now; activates with the booking-data migration. Safe (additive; no change for valid input). No console errors across the live routes.
- **#39 → reverted; new finding #43.** While implementing #39 I found its target — the in-app feedback modal (`openFeedbackModal`) — is **dead code: defined but never invoked** (the footer "Send feedback" opens an external MS Form). Adding validation to an unreachable function is pointless, so I reverted that edit and logged **#43** (code-health, same class as the removed #17 Quick Match): the AEM team should wire `openFeedbackModal` to a trigger or drop it. Not removed unilaterally — whether the in-app modal should supersede the MS Form is a product call.

**This exhausts the safe auto-fixable list.** Fixed across iters 12–16: **9 of the 15** review-doc Lows (#29, #30, #32, #33, #34, #35, #36, #38, #40). Genuinely remaining: **#31** (pagination-in-URL — product/behavioral choice), **#37** (Lab2Market Calendly → institutional — *coordinator data*), **#41/#42** (NCV builder — accepted-open **#12**), **#43** (dead-modal — wire-or-drop product call). Nothing else is responsibly auto-fixable on the frozen build; subsequent loop passes will be verification/idle only. **Unpushed stack: 8 commits** (`7fe58bf` → this) — a push is the highest-value next action.
