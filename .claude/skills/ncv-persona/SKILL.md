---
name: ncv-persona
description: Persona-driven QA loop for the Narrative-CV prototype (current target: V5 beta). Each invocation plays ONE researcher persona through narrative-cv-prototype-v5.html in the browser pane — really typing and clicking — aiming to end with a decent draft, logging every hiccup. Log-only, never fixes bugs. Use when the user types /ncv-persona <id|next|list>, says "run a persona", "persona-test the NCV tool", or asks to play a researcher through the prototype.
---

# ncv-persona — play a researcher through the tool (V5 beta is the current target)
...
Personas live in `tests/ncv-personas/personas-v4.json` (roster is version-agnostic); reports land
in `tests/ncv-personas/v5-sessions/`. **Version:** the current target is
`narrative-cv-prototype-v5.html`, localStorage key `ncv-v5`. To run against legacy V4, the user
must say so explicitly — then use `narrative-cv-prototype-v4.html`, key `ncv-v4`, and
`v4-sessions/`.

## Hard rules

- **Log-only.** Never edit the prototype under test or any product file. Fixes are
...
- **Never click "Reset draft"** (native `confirm()` can hang the pane) — clear state via
  `localStorage.removeItem('ncv-v5')` + reload. **Never click the download buttons or
...
- Runs must be comparable: record the prototype's git SHA; if
  `git status --porcelain` on the prototype under test is dirty, stop and tell the user.
...
- `/ncv-persona list` — table of roster ids (roster order) with stage, agency, entry mode,
  and last run date (from `ls tests/ncv-personas/v5-sessions/`), then stop.
- `/ncv-persona next` — run the first roster-order persona with no
  `v5-sessions/<id>-*.md` report. If none remain → print `LOOP STATUS: dry — stop`.
...
2. **Server.** preview_start `{name: "pathways-review"}` (port 8765 — never the user's
   :8000), navigate to `http://localhost:8765/narrative-cv-prototype-v5.html`.
3. **Clean slate.** `localStorage.removeItem('ncv-v5')`, reload, verify blank (mode cards
...
5. **End-state capture** (no downloads): the Review stage's readonly export textarea value
   ...
   `localStorage.getItem('ncv-v5')` pretty-printed to
   `tests/ncv-personas/v5-sessions/<id>-<date>.state.json`.
...
7. **Report + log.** Write `tests/ncv-personas/v5-sessions/<id>-<YYYY-MM-DD>.md` using the
