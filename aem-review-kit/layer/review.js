/* aem-review-kit — review layer for a review copy of an AEM page.
   Canonical copy: aem-review-kit/layer/review.js. The build copies it into each guide folder; edit it here.
   - Reviewer view: the corrected page and a highlight-and-comment layer (Open / Resolved).
   - AEM copy mode (switch, or ?aem=1): every change tagged with Copy text / Copy HTML, any block copyable,
     and a To do / Done checklist of what has been pasted into AEM ("done" marks carry a text fingerprint).
   ...
   - Storage: RV_CONFIG.storage.backend = "local" (this browser only) or "sheet" (Apps Script webhook).
*/
(function () {
  "use strict";
  ...
  const GROUPS = CFG.groups || { A: "Build error", B: "Factual, verified", C: "Copy" };
  ...
  const store = {
    ...
    page: (CFG.storage && CFG.storage.page) || "guide",
    key: "rv.rows." + ((CFG.storage && CFG.storage.page) || "guide"),
    async load() {
  ...
  function renderBar() {
    ...
    bar.innerHTML = '<span class="rv-bar__title">Review copy · ' + esc(CFG.title || "guide") + "<small>" + esc(CFG.snapshot || "") + "</small></span>"
